 ...
 */
export const getSkyObjectsOverheadTool = {
    name: 'get_sky_objects_overhead',
    description: 'Get the Sun, Moon and all major planets currently in the sky for a location, with altitude (degrees above horizon), azimuth (compass bearing), distance and visibility. Use this for "what planets are visible right now", "is the Moon up", or "what is above me" questions.',
    parameters: {
        type: 'object',
        properties: {
    ...
    handler: async (args) => {
        const bodies = astronomyService.getSkyOverhead({ latitude: args.latitude, longitude: args.longitude }, new Date(), { onlyVisible: !!args.onlyVisible });
        return {
            count: bodies.length,
            bodies: bodies.map((b) => ({
                name: b.name,
...
export const getPlanetPositionTool = {
    name: 'get_planet_position',
    description: 'Get the precise sky position of a single celestial body (a planet, the Moon, or the Sun) for a location: altitude, azimuth, distance, visibility, illumination, and next rise/set times. Use this for "where is Mars right now" or "when does Jupiter rise" questions.',
    parameters: {
        type: 'object',
        properties: {
...
function compassFromAzimuth(azimuth) {
